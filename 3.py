...
if deals:
    ...
    signals = pd.Series(np.where((df['ema_fast'].shift(1) <= df['ema_slow'].shift(1)) & (df['ema_fast'] > df['ema_slow']), 1,  # Buy
                        ...
                        index=df.index)
    # Use open price of next bar for return calculation to simulate entry
    returns = (df['open'].shift(-1) - df['close']) / df['close'] * signals.shift(-1)  # Return based on next open
